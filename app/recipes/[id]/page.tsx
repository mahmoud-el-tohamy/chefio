...
function LikeButton({ initialLiked, initialCount, recipeId, isOwner }: { initialLiked: boolean; initialCount: number; recipeId: string; isOwner: boolean }) {
  ...
  if (isOwner) {
    return null; // Don't render the like button if user is the owner
  }
...
function EditButton({ recipeId, isOwner }: { recipeId: string; isOwner: boolean }) {
  const router = useRouter();
  ...
  return (
    <button
      className={`${styles.editRecipe} ${!isOwner ? styles.hidden : ''}`}
...
function RecipeHeader({ recipe }: { recipe: Recipe }) {
  ...
  const handleProfileClick = async (e: React.MouseEvent) => {
    ...
    router.push(`/profile/${recipe.createdBy.username}`);
  };
...
export default function RecipePage() {
  ...
  return (
    <>
      <Head>
        <title>{recipe.foodName} - Recipe</title>
      </Head>
      <Navbar />
      <main className={styles.main}>
